...
int RangeCount(BSTNode* root, int grade1, int grade2)
{
    ...
    if (grade1 <= root->grade && root->grade <= grade2)
        cen += root->num;
    if (root->left_child == NULL && root->right_child == NULL)
        return cen;
    else
    {
        if (root->left_child != NULL && root->grade > grade1) l = RangeCount(root->left_child, grade1, grade2);
        if (root->right_child != NULL && root->grade < grade2) r = RangeCount(root->right_child, grade1, grade2);
        return cen + l + r;
    }
}
// Get the grade of the n-th highest valuable diamond
