...
int main(void)
{
    ...
    scanf("%s", parenArr);
    printf("%d", CalParenVal(parenArr, strlen(parenArr)));
    return 0;
}
...
bool IsRight(Stack *pstack, char paren)
{
    return ( ParenKind(pstack->items[pstack->top]) == ParenKind(paren) - 3 );
}

int BlockLen(char pParenArr[], int len)
{
    Stack stack;
    char target = (pParenArr[0] == '(') ? ')' : (pParenArr[0] == '{') ? '}' : ']';
    int i;
    InitStack(&stack);
    for (i = 0; i < len; i++)
    {
...
int CalParenVal(char pParenArr[], int len)
{
    ...
    for (i = 0; i < len; i++)
    {
        if ( ParenKind(pParenArr[i]) < 3 ) // open paren?
        {
            if ( i == 0 )
                Push(&stack, pParenArr[i]);
            else
            {
                if ( ParenKind(pParenArr[i - 1]) >= 3 ) // close paren?
                {
